...
const publicFiles = (path)=>{
    return `${process.env.NEXT_PUBLIC_BACKEND_URL}/public/${path}`
}
const projectBundles = (url)=>{
    return `${process.env.NEXT_PUBLIC_BACKEND_URL}/bundle/${url}`
}
const bannerFiles = (path)=>{
    return `${process.env.NEXT_PUBLIC_BACKEND_URL}/banner/${path}`
}
